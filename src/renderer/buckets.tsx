...
import { Bar, Btn, Icon, Panel, StatusPill, TipBtn } from './ui/primitives';
...
function BucketPanel({ bucket, state }: { bucket: BucketView; state: State }) {
  ...
  const actions = (
    <>
      ...
      {running ? (
        ...
        <Btn variant="primary" onClick={(e) => act(() => window.sb.bucketAction(bucket.id, 'start'), e.currentTarget)}>
          Start bucket
        </Btn>
      )}
  ...
  return (
    <Panel
      ...
      title={bucket.name}
      status={<StatusPill on={running}>{bucket.status}</StatusPill>}
      meta={
        users.length
